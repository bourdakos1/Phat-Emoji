...
var observer = new MutationObserver(function(mutations, observer) {
    ...
    Promise.all(promises).then(function(results) {
        ...
        //     var message = encrypt[i].innerText;
        //     if (message.split('::::::')[1] != null) {
        //         var decrypted = new dcodeIO.ByteBuffer.wrap(results[]).toString('binary');
        //         encrypt[i].innerHTML = '<div style="display: flex;"><div style="flex: 0; margin-right:10px"><img alt="🔑" class="_1ift _2560 img" src="https://static.xx.fbcdn.net/images/emoji.php/v9/z4c/2/16/1f511.png"></div><div style="word-break: break-all; width: 100%;" class="decrypted_message" style="flex: 1"></div>';
        //         encrypt[i].getElementsByClassName("decrypted_message")[0].innerHTML = htmlEncode(decrypted).replace(/\*(\S(.*?\S)?)\*/gm, '<b>$1</b>').replace(/_(\S(.*?\S)?)_/gm, '<i>$1</i>').replace(/~(\S(.*?\S)?)~/gm, '<del>$1</del>')
        //     }
        // }
    });
});
